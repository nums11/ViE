    ...
    <div v-else id="right-section" class="inline-block">
      ...
      <sui-form class="form">
        ...
        <sui-button
          ...
          "
        >
          <sui-button-content visible> Add real-time tasks </sui-button-content>
...
import NewMeetingPortionContainer from "@/components/NewMeetingPortionContainer.vue";
...
export default {
  ...
  computed: {
    formComplete() {
      return this.meeting.title !== "" && this.meeting.sections.length > 0;
    },
  },
  created() {
    this.getCourse();
  ...
  methods: {
    ...
    showAddTaskModal(task_type) {
      if (task_type === "real-time") this.$refs.RealTimeModal.showModal();
      else if (task_type === "async") this.$refs.AsyncModal.showModal();
    },
    hideAddTaskModal() {
    ...
    addTask(task_type, task) {
      if (task_type === "qr_scan") this.real_time_portion.qr_scans.push(task);
      else this.async_portion.videos.push(task);
      this.hideAddTaskModal();
    },
    selectSection(section) {
      let [meeting_has_section, index] = this.meetingHasSection(section);
      let section_container = this.getSectionContainer(section.section_number);
    ...
    meetingHasSection(section) {
      ...
      for (let i = 0; i < this.meeting.sections.length; i++) {
        if (this.meeting.sections[i] === section._id) {
    ...
    removeTask(task_type, index) {
      ...
      else this.async_portion.videos.splice(index, 1);
    },
    ...
    async createMeeting() {
      try {
        ...
        let async_portion;
        if (
          this.portionTimesSet(true) &&
          this.real_time_portion.qr_scans.length > 0
        )
          real_time_portion = this.real_time_portion;
        else real_time_portion = null;
        if (this.portionTimesSet(false) && this.async_portion.videos.length > 0)
          async_portion = this.async_portion;
        else async_portion = null;
        if (async_portion != null) {
          const videos_with_urls = await this.saveVideosToGCS();
          if (videos_with_urls == null) throw "Error saving videos to GCS";
          async_portion.videos = videos_with_urls;
        }
        const response = await MeetingAPI.addMeeting(
          this.meeting,
          real_time_portion,
          async_portion,
          this.state_user._id
        ...
        this.$router.push({
          name: "meeting_info",
          params: { meeting_id: saved_meeting._id },
        });
      } catch (error) {
        console.log(error);
        ...
        this.async_portion.videos.forEach((video) => {
          video_promises.push(
            new Promise(async (resolve, reject) => {
              try {
                const response = await MeetingAPI.saveVideoToGCS(
                  video.video_file
                );
