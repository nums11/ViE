...
describe('Meeting Creation', () => {
  ...
  beforeEach(() => {
    ...
    cy.get('.venue-navbar')
      .contains('RCOS', {timeout: 10000}).click({force: true})
  ...
  it('Can create a meeting with a live task', () => {
    ...
    cy.get('#live-section .meeting-info-pill', {timeout: 3000})
      .should('contain','Meeting with live task')
      .and('contain', 'CSCI 2961').and('contain', 'RCOS')
    cy.get('.venue-navbar')
      .contains('RCOS', {timeout: 5000}).click({force: true})
  ...
  it('Can create a meeting with an async task',() => {
    ...
    cy.contains('Create New Meeting').click()
    cy.url({timeout: 20000}).should('contain', 'meeting_info')
    ...
    cy.contains('Dashboard').click({force: true})
    cy.get('#async-section .meeting-info-pill', {timeout: 3000})
      .should('contain','Meeting with async task')
      .and('contain', 'CSCI 2961').and('contain', 'RCOS')
    cy.get('.venue-navbar')
      .contains('RCOS', {timeout: 5000}).click({force: true})
    cy.get('.meeting-attendance-pill', {timeout: 10000}).
      contains('Meeting with async task')
  })
